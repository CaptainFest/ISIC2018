...
def prepare_data():
    ...
    arg('--svpath', type=str, default='/home/irek/My_work/train/h5_all/')
    arg('--jobs', type=int, default=12)
    ...
    results = Parallel(n_jobs=args.jobs)(delayed(load_image)(ind, row) for ind, row in enumerate(img_inds))
...
def load_image(ind, img_id, args):
    ...
    masks[masks == 0] = -1
    hdf5_file = h5py.File(args.svpath + '%s_attribute_all.h5' % (img_id), 'w')
