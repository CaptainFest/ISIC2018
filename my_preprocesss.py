...
mask_path = "/home/fest/ISIC/binary/"
...
def load_image(ind, img_id):
    ...
    for i, attr in enumerate(attr_types):
        ...
        m = load_img(mask_file, target_size=(224, 224), color_mode="grayscale")  # this is a PIL image
        m_np = img_to_array(m)
        masks[:, :, i] = m_np[:, :, 0]
...
results = Parallel(n_jobs=2)(delayed(load_image)(ind,row) for ind,row in enumerate(img_inds))
